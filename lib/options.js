...
/**
 * Initialize and validate tokenRegex
 * @param  {string} tokenRegex - regex for splitting inputString
 * @return {RegExp} initialized and validated tokenRegex
 *
 * @example
 * getTokenRegex(', ') // => new RegExp(', ', 'g')
 * getTokenRegex('') // => new RegExp('', 'g')
 * getTokenRegex() // => new RegExp(' ', 'g')
 */
function getTokenRegex(tokenRegex) {
    const value = tokenRegex || tokenRegex === '' ? tokenRegex : ' ';
    return new RegExp(
        Validator
            .init('tokenRegex', value)
            .isStr()
            .isRequired()
            .value,
        'g'
 ...
 *
 * @example
 * get({numLines: 0, lengthOfLine: 0, firstLineRequired: false, tokenRegex: ', '})
 * // => {numLines: 1, lengthOfLine: [0], firstLineRequired: false, tokenRegex: new RegExp(', ', 'g')}
 * get({}) // => {numLines: 1, lengthOfLine: [255], firstLineRequired: true, tokenRegex: new RegExp(' ', 'g')}
 */
function get(options) {
    return {
        ...
        tokenRegex: getTokenRegex(options.tokenRegex)
    };
}
